...
In the [last post](./Components-Props-State.md), we’ve covered components, props and state.
...
See the Pen [React House With Cat](https://codepen.io/focuser/pen/VydPEJ/) by focuser ([@focuser](https://codepen.io/focuser/)) on [CodePen](https://codepen.io/).
...
That is right. We cannot directly change the state of House here. However, nothing prevents us from doing this indirectly. See this…
...
At this point, we haven’t specified when to run this piece of code. We just assign it a name (“toggleDoorStatus”) so that we can pick it by name and run it later.
